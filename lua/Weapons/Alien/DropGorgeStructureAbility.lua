...
class 'DropGorgeStructureAbility' (Ability)

local kMaxStructuresPerType = 20
local kDropCooldown = 1

DropGorgeStructureAbility.kMapName = "drop_gorgestructure_ability"
...
DropGorgeStructureAbility.kSupportedStructures = { ClogAbility, WhipStructureAbility, CragStructureAbility, ShiftStructureAbility, ShadeStructureAbility }
...
function DropGorgeStructureAbility:GetAnimationGraphName()
    return kAnimationGraph
end

function DropGorgeStructureAbility:GetActiveStructure()

    if self.activeStructure == nil then
        return nil
    else
        return DropGorgeStructureAbility.kSupportedStructures[self.activeStructure]
    end

end

function DropGorgeStructureAbility:OnCreate()
...
function DropGorgeStructureAbility:GetDeathIconIndex()
    return kDeathMessageIcon.Consumed
end

function DropGorgeStructureAbility:SetActiveStructure(structureNum)
...
function DropGorgeStructureAbility:GetHasDropCooldown()
    return self.timeLastDrop ~= nil and self.timeLastDrop + kDropCooldown > Shared.GetTime()
end

function DropGorgeStructureAbility:GetSecondaryTechId()
    return kTechId.Spray
end

function DropGorgeStructureAbility:GetNumStructuresBuilt(techId)

    if techId == kTechId.GorgeShift then
        return self.numShiftsLeft
    end
    
    if techId == kTechId.GorgeShade then
    ...
    if techId == kTechId.GorgeCrag then
...
function DropGorgeStructureAbility:OnPrimaryAttack(player)
...
function DropGorgeStructureAbility:OnPrimaryAttackEnd(player)
...
function DropGorgeStructureAbility:GetIsDropping()
    return self.dropping
end

function DropGorgeStructureAbility:GetEnergyCost(player)
    return kDropStructureEnergyCost
end

function DropGorgeStructureAbility:GetDamageType()
...
function DropGorgeStructureAbility:OnSecondaryAttack(player)
...
function DropGorgeStructureAbility:GetSecondaryEnergyCost(player)
    return 0
end

function DropGorgeStructureAbility:PerformPrimaryAttack(player)
    ...
    if secondClick then
        ...
        if valid then
            ...
            if player:GetResources() >= cost and not self:GetHasDropCooldown() then
                ...
                local message = BuildGorgeDropGorgeStructureMessage(player:GetEyePos(), player:GetViewCoords().zAxis, self.activeStructure, self.lastClickedPosition)
                Client.SendNetworkMessage("GorgeBuildGorgeStructure", message, true)
        ...
        if valid and enoughRes and structureAbility:IsAllowed(player) and enoughEnergy and not self:GetHasDropCooldown() then
            ...
            if structure then
                ...
                if structure:SpaceClearForEntity(coords.origin) then
                    ...
                    elseif structure:isa("Crag") then
...
function DropGorgeStructureAbility:OnDropStructure(origin, direction, structureIndex, lastClickedPosition)

    local player = self:GetParent()
        
    if player then
    
        local structureAbility = DropGorgeStructureAbility.kSupportedStructures[structureIndex]        
...
function DropGorgeStructureAbility:CreateStructure(coords, player, structureAbility, lastClickedPosition)
...
function DropGorgeStructureAbility:GetPositionForStructure(startPosition, direction, structureAbility, lastClickedPosition)
   ...
   if requiresInfestation then SetCachedTechData(structureAbility.GetDropStructureId(), kTechDataRequiresInfestation, false) end
...
function DropGorgeStructureAbility:GetIgnoreGhostHighlight()
...
function DropGorgeStructureAbility:OnDraw(player, previousWeaponMapName)
...
function DropGorgeStructureAbility:OnTag(tagName)
    if tagName == "shoot" then
        self.dropping = false
    end
end

function DropGorgeStructureAbility:OnUpdateAnimationInput(modelMixin)

    PROFILE("DropGorgeStructureAbility:OnUpdateAnimationInput")
...
function DropGorgeStructureAbility:ProcessMoveOnWeapon(input)
    ...
    if player then
        ...
        if Server then
            ...
            local numAllowedShifts = LookupTechData(kTechId.GorgeShift, kTechDataMaxAmount, -1) 
            local numAllowedShades = LookupTechData(kTechId.GorgeShade, kTechDataMaxAmount, -1) 
            local numAllowedWhips = LookupTechData(kTechId.GorgeWhip, kTechDataMaxAmount, -1) 
            local numAllowedClogs = LookupTechData(kTechId.Clog, kTechDataMaxAmount, -1) 
            local numAllowedCrags = LookupTechData(kTechId.GorgeCrag, kTechDataMaxAmount, -1) 
            if numAllowedShifts >= 0 then     
                self.numShiftsLeft = team:GetNumDroppedGorgeStructures(player, kTechId.GorgeShift)           
            end
               if numAllowedShades >= 0 then     
                self.numShadesLeft = team:GetNumDroppedGorgeStructures(player, kTechId.GorgeShade)           
            end
              if numAllowedWhips >= 0 then     
                self.numWhipsLeft = team:GetNumDroppedGorgeStructures(player, kTechId.GorgeWhip)           
            end
            ...
            if numAllowedCrags >= 0 then     
                self.numCragsLeft = team:GetNumDroppedGorgeStructures(player, kTechId.GorgeCrag)           
            end
...
function DropGorgeStructureAbility:GetShowGhostModel()
    return self.activeStructure ~= nil and not self:GetHasDropCooldown()
end

function DropGorgeStructureAbility:GetGhostModelCoords()
    return self.ghostCoords
end   

function DropGorgeStructureAbility:GetIsPlacementValid()
    return self.placementValid
end

function DropGorgeStructureAbility:GetGhostModelTechId()
...
function DropGorgeStructureAbility:GetGhostModelName(player)
...
if Client then
    ...
    function DropGorgeStructureAbility:OnProcessIntermediate(input)
    ...
    function DropGorgeStructureAbility:CreateBuildMenu()
    
        if not self.buildMenu then        
            self.buildMenu = GetGUIManager():CreateGUIScript("GUIGorgeStructBuildMenu")            
        end
        
    end
    
    function DropGorgeStructureAbility:DestroyBuildMenu()
    ...
    function DropGorgeStructureAbility:OnDestroy()
    ...
    function DropGorgeStructureAbility:OnKillClient()
        self.menuActive = false
    end
    
    function DropGorgeStructureAbility:OnDrawClient()
    ...
    function DropGorgeStructureAbility:OnHolsterClient()
    ...
    function DropGorgeStructureAbility:OnSetActive()
    end
    
    function DropGorgeStructureAbility:OverrideInput(input)
    ...
    function DropGorgeStructureAbility:OnUpdateRender()
        UpdateGUI(self, self:GetParent())    
    end
    
end

Shared.LinkClassToMap("DropGorgeStructureAbility", DropGorgeStructureAbility.kMapName, networkVars)
