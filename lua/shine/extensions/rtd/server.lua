...
if MarineorJetpackMarineorExoRoll == 2 then
      ...
      self:CreateTimer(7, 1, kNerveGasAmount, function () if not Player:GetIsAlive() then self:DestroyTimer(7) self.ScreenTextEnd(58) return end Player:GiveItem(GasGrenade.kMapName) end )
      return 
  ...
  //self:NotifyAlien( nil, "%s Player is an alive alien, not commander.", true, Player:GetName())
   ...
   local playermaxarmor = Player:GetMaxArmor()
     ...
     if AlienRoll == 2 then 
      ...
      self:AddDelayToPlayer(Player) 
      local enzymeamount = math.random(1,8)
      Shine.ScreenText.Add( "EnzymeText", {X = 0.20, Y = 0.80,Text = "EnzymeCloud: %s",Duration = enzymeamount * 8,R = math.random(0,255), G = math.random(0,255), B = math.random(0,255),Alignment = 0,Size = 1,FadeIn = 0,}, Player )
      self:NotifyAlien( nil, "%s won %s enzyme clouds, 1 every 8 seconds (or until dead)", true, Player:GetName(), enzymeamount)  
      Player:GiveItem(EnzymeCloud.kMapName)
      self:CreateTimer("Enzyme", 8, enzymeamount, function () if not Player:GetIsAlive() then self:DestroyTimer("Enzyme") self.ScreenText.End("EnzymeText") return end Player:GiveItem(EnzymeCloud.kMapName) end ) 
     ...
     Shine.ScreenText.Add( 61, {X = 0.20, Y = 0.80,Text = "Electrified: %s",Duration = kElectrifyTimer,R = math.random(0,255), G = math.random(0,255), B = math.random(0,255),Alignment = 0,Size = 1,FadeIn = 0,}, Player )
      ...
      self:CreateTimer(14, 8, amount, function () if not Player or not Player:GetIsAlive() then self:DestroyTimer(14) self.ScreenText.End(70) return end Player:GiveItem(HallucinationCloud.kMapName) end )
      ...
      Shine.ScreenText.Add( 64, {X = 0.20, Y = 0.80,Text = "Contamination: %s",Duration = amount * 4,R = math.random(0,255), G = math.random(0,255), B = math.random(0,255),Alignment = 0,Size = 1,FadeIn = 0,}, Player )
      ...
      self:CreateTimer(17, 4, amount, function () if not Player:GetIsAlive() then self:DestroyTimer(17) self.ScreenText.End(64) return end Player:GiveItem(Contamination.kMapName) end ) 
      ...
      if EffectsRoll == 11 then
      local amount = math.random(1,8)
      Shine.ScreenText.Add( "InkTimer", {X = 0.20, Y = 0.80,Text = "ShadeInk: %s",Duration = amount * 8,R = math.random(0,255), G = math.random(0,255), B = math.random(0,255),Alignment = 0,Size = 1,FadeIn = 0,}, Player )
      ...
      self:CreateTimer("Ink", 8, amount, function () if not Player:GetIsAlive() then self:DestroyTimer("Ink") self.ScreenText.End("InkTimer") return end Player:GiveItem(ShadeInk.kMapName) end ) 
